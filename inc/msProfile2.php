...
class msProfile2 extends msProfile {
  function msProfile2(&$msLoader) {
    $this->type = 'msProfile2';
  ...
  public function userName() {
    $regs = $this->regex('%<div class="basicInfoDetails">[\w\W\s]*?<h2.*?>(.*?)</h2>%i', $this->html);
  ...
  public function userDefaultImage() {
    $regs = $this->regex('%<div class="photo">[\w\W\s]*?<img.*?class="photo.*?".*?src="(.*?)" />%i', $this->html);
    $return = $regs[0][1];
  ...
  public function userAboutMe() {
    $regs = $this->regex('%<div class="autoResize blurbAboutMe">[\s]*?<h4>About me:</h4>[\s]*?([\w\W\s]*?)[\s]*?</div>[\s]*?<div class="autoResize blurbLikeToMeet">%i', $this->html);
    if ($regs) {
